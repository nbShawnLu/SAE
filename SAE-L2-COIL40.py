...
startfrom = [0,0,0]
mm= 0
for reg2 in [0.5,1,2,3,4,6,10,20,30,80,150]:
	for reg3 in [0.5,1,2,3,4,6,8,10,20,30,60,100]:
		for learning_rate in [1e-4,2e-4,3e-4,5e-4]:
			try:
				...
				for i in range(0,1):
					...
					for iter_ft in range(ft_times):
						cost, C = CAE.partial_fit(coil100_all_subjs, learning_rate, mode='fine')  #
					...
					for posti in range(3):
						drawC(C)
						acc = display(C, coil100_all_subjs, d, alpha, ro, num_class,label_all_subjs)
						acc_.append(acc)
					acc_.append(acc)
